...
int INRINEX;
//@endcond 
///A data type to define a time interval
...
int main(int argc, char* argv[]) {
	...
	if (!aStr.empty()) {
		...
		timeInterval.fromTimeTag = getInstantGNSStime(week, tow);
	...
	if (!aStr.empty()) {
		...
		getWeekTowGPSdate (year, month, day, hour, minute, second, week, tow);
		timeInterval.toTimeTag = getInstantGNSStime(week, tow);
    ...
    //convert obsV2Tokens to V3 and append them to obsTokens using equivalence tables defined in RinexData.h
    string observable;
	...
	FILE* outFile;
	size_t anIdx;
	while ((anIdx = fileName.find('.')) != string::npos) fileName.replace(anIdx, 1, 1, '_'); 	//replace . by _ in fileName
...
int generateObsCSV(FILE *inFile, FILE* outFile, RinexData &rinex, TimeIntervalParams &timeInterval, Logger* plog) {
	...
	try {
		...
		while ((rdStat = rinex.readObsEpoch(inFile)) != 0) {
			if (rdStat == 1 &&
                    timeInInterval(rinex.getEpochTime(week, tow, aDouble, anInt), timeInterval) &&
                    rinex.filterObsData()) {
                //Epoch observables and data are well formatted, and it remains data after filtering
				nrec++;
				for (unsigned int index = 0; rinex.getObsData(sys, sat, obsType, value, lol, strg, index); index++) {
